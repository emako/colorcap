...
{
    this->setWindowTitle(QString("CapColor %1").arg(SOFT_VERSION));
...
{
    for(int i = 0; i < (int)ReflashItem::max; i++)
    {
        is_reflash[i] = true;
    }
}
...
void MainWindow::mouseReleaseEvent(QMouseEvent * event)
{
    ...
    {
        unsetCursor();
        ui->eyedropperBt->setStyleSheet("background-image: url(:/res/eyedropper_tool_32.png);");
    }
    else
    {
        /*プレビュー画像をクリック*/
        ...
        if(rect.contains(event->pos()))
        {
            yuv_format = (YuvFormat)!(bool)yuv_format;
            if((bool)yuv_format)
            {
        ...
        if(rect.contains(event->pos()))
        {
            if(mYuvDialog->isHidden())
            {
                ...
                if(mYuvDialog->exec() == QDialog::Accepted)
                {
                    ...
                    is_reflash[(int)ReflashItem::yuv_yuv] = false;
                    updateColor(color);
                    is_reflash[(int)ReflashItem::yuv_yuv] = true;
                }
                else
                {
                    /*Do nothing*/;
...
void MainWindow::on_eyedropperBt_pressed()
{
    ...
    setCursor(QPixmap(":/res/eyedropper_tool_24.png"));
#ifdef EYEDROPPER_CUR
    setCursor(QPixmap(":/res/eyedropper.cur"));
#else
    ui->eyedropperBt->setStyleSheet("background-image: url(:/res/blank_alpha.png);");
#endif
}
...
inline QColor MainWindow::yuvToRgbColor(YuvFormat format)
{
    ...
    case YuvFormat::BT601:
        ...
        r = 1.164 * ((float)y - 16.f) + 1.596 * ((float)v - 128.f);
        g = 1.164 * ((float)y - 16.f) - 0.391 * ((float)u - 128.f) - 0.813 * ((float)v - 128.f);
        b = 1.164 * ((float)y - 16.f) + 2.018 * ((float)u - 128.f);
        break;
    case YuvFormat::BT709:
    default:
        /*YPbPr -> RGB (BT.709 HDTV)*/
        r = 1.164 * ((float)y - 16.f) + 1.793 * ((float)v - 128.f);
        g = 1.164 * ((float)y - 16.f) - 0.213 * ((float)u - 128.f) - 0.533 * ((float)v - 128.f);
        b = 1.164 * ((float)y - 16.f) + 2.112 * ((float)u - 128.f);
        break;
    }
    r = qBound(0, r, 255);
    g = qBound(0, g, 255);
...
{
    QMessageBox::about(this, tr("About"), Common::fromResource(":/res/strings/about").arg(SOFT_VERSION));
}
...
inline void MainWindow::updateColor(QColor color)
{
    ...
    /*RGB値を設置*/
    if(is_reflash[(int)ReflashItem::rgb_rgb])
    {
        ui->rgbEdit->setText(toRgbColor(color));
    }
    if(is_reflash[(int)ReflashItem::ass_rgb])
    {
        ui->assEdit->setText(toAssColor(color));
    }
    if(is_reflash[(int)ReflashItem::html_rgb])
    {
        ui->htmlEdit->setText(toHtmlColor(color));
    }

    if(is_reflash[(int)ReflashItem::r_rgb])
    {
        ui->redBox->setValue(color.red());
    }
    if(is_reflash[(int)ReflashItem::g_rgb])
    {
        ui->greenBox->setValue(color.green());
    }
    if(is_reflash[(int)ReflashItem::b_rgb])
    {
        ui->blueBox->setValue(color.blue());
    }

    /*YUV値を設置*/
    if(is_reflash[(int)ReflashItem::yuv_yuv])
    {
        ui->yuvEdit->setText(toYuvColor(color));
    }
    if(is_reflash[(int)ReflashItem::y_yuv])
    {
        ui->y_yuvBox->setValue(y);
    }
    if(is_reflash[(int)ReflashItem::u_yuv])
    {
        ui->u_yuvBox->setValue(u);
    }
    if(is_reflash[(int)ReflashItem::v_yuv])
    {
        ui->v_yuvBox->setValue(v);
    }

    /*HSV/HSB値を設置*/
    if(is_reflash[(int)ReflashItem::hsv_hsv])
    {
        ui->hsvEdit->setText(toHsvColor(color));
    }
    if(is_reflash[(int)ReflashItem::h_hsv])
    {
        ui->h_hsvBox->setValue(color.hsvHue());
    }
    if(is_reflash[(int)ReflashItem::s_hsv])
    {
        ui->s_hsvBox->setValue(color.hsvSaturation());
    }
    if(is_reflash[(int)ReflashItem::v_hsv])
    {
        ui->v_hsvBox->setValue(color.value());
    }

    /*HSL値を設置*/
    if(is_reflash[(int)ReflashItem::hsl_hsl])
    {
        ui->hslEdit->setText(toHslColor(color));
    }
    if(is_reflash[(int)ReflashItem::h_hsl])
    {
        ui->h_hslBox->setValue(color.hslHue());
    }
    if(is_reflash[(int)ReflashItem::s_hsl])
    {
        ui->s_hslBox->setValue(color.hslSaturation());
    }
    if(is_reflash[(int)ReflashItem::l_hsl])
    {
...
void MainWindow::on_redBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::r_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::r_rgb] = true;
}
...
void MainWindow::on_greenBox_valueChanged(int arg1)
{
    ...
    color.setGreen(arg1);
    color.setBlue(ui->blueBox->value());
    is_reflash[(int)ReflashItem::g_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::g_rgb] = true;
}
...
void MainWindow::on_blueBox_valueChanged(int arg1)
{
    ...
    color.setGreen(ui->greenBox->value());
    color.setBlue(arg1);
    is_reflash[(int)ReflashItem::b_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::b_rgb] = true;
}
...
void MainWindow::on_assEdit_textEdited(const QString &arg1)
{
    ...
    is_reflash[(int)ReflashItem::ass_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::ass_rgb] = true;
}
...
void MainWindow::on_htmlEdit_textEdited(const QString &arg1)
{
    ...
    is_reflash[(int)ReflashItem::html_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::html_rgb] = true;
}
...
void MainWindow::on_rgbEdit_textEdited(const QString &arg1)
{
    ...
    is_reflash[(int)ReflashItem::rgb_rgb] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::rgb_rgb] = true;
}
...
void MainWindow::on_h_hsvBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::h_hsv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::h_hsv] = true;
}
...
void MainWindow::on_s_hsvBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::s_hsv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::s_hsv] = true;
}
...
void MainWindow::on_v_hsvBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::v_hsv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::v_hsv] = true;
}
...
void MainWindow::on_hsvEdit_textEdited(const QString &arg1)
{
    ...
    is_reflash[(int)ReflashItem::hsv_hsv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::hsv_hsv] = true;
}
...
void MainWindow::on_h_hslBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::h_hsl] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::h_hsl] = true;
}
...
void MainWindow::on_s_hslBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::s_hsl] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::s_hsl] = true;
}
...
void MainWindow::on_l_hslBox_valueChanged(int arg1)
{
    ...
    is_reflash[(int)ReflashItem::l_hsl] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::l_hsl] = true;
}
...
void MainWindow::on_hslEdit_textEdited(const QString &arg1)
{
    ...
    is_reflash[(int)ReflashItem::hsl_hsl] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::hsl_hsl] = true;
}
...
void MainWindow::on_y_yuvBox_valueChanged(int arg1)
{
    ...
    v = ui->v_yuvBox->value();
    QColor color = yuvToRgbColor(yuv_format);
    is_reflash[(int)ReflashItem::y_yuv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::y_yuv] = true;
}
...
void MainWindow::on_u_yuvBox_valueChanged(int arg1)
{
    ...
    v = ui->v_yuvBox->value();
    QColor color = yuvToRgbColor(yuv_format);
    is_reflash[(int)ReflashItem::u_yuv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::u_yuv] = true;
}
...
void MainWindow::on_v_yuvBox_valueChanged(int arg1)
{
    ...
    v = arg1;
    QColor color = yuvToRgbColor(yuv_format);
    is_reflash[(int)ReflashItem::v_yuv] = false;
    updateColor(color);
    is_reflash[(int)ReflashItem::v_yuv] = true;
}
